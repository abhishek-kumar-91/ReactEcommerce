...
const lowToHigh = () =>{
  setShowData(itemData)
  const result = itemData.filter(item=>  item.price < "100")
  ...
  return (
    <>
     ...
     <div className='mt-5 flex'>
      <div><h1 className='font-bold text-lg text-gray-500'>Sort: By Price</h1></div>
      <div><button onClick={()=>lowToHigh()} className='ml-10 text-md font-semibold hover:text-orange-500 hover:scale-105 duration-300 ease-linear'>Low Price</button></div>
      <div><button onClick={()=>highToLow()} className='ml-10 text-md font-semibold hover:text-orange-500 hover:scale-105 duration-300 ease-linear'>High Price</button></div>
     </div>
    </div>
